...
static char *GetRegistryString(registry_value_t * reg_val)
{
	...
	if (RegQueryValueEx
	    (key, reg_val->value, NULL, &valtype, (unsigned char *)result, &len)
	    != ERROR_SUCCESS) {
		free(result);
		return NULL;
	}
...
static void CheckCollectorsEdition(void)
{
	...
	for (i = 0; i < arrlen(collectors_edition_subdirs); ++i) {
		...
		sprintf(subpath, "%s\\%s", install_path,
...
static char *SearchDirectoryForIWAD(char *dir)
{
	...
	for (i = 0; i < arrlen(iwads); ++i) {
		...
		if (!strcmp(dir, ".")) {
			...
			sprintf(filename, "%s%c%s", dir, DIR_SEPARATOR,
				iwadname);
		}
...
static char *GetGameName(char *gamename)
{
	...
	for (i = 0; i < arrlen(banners); ++i) {
		...
		if (deh_sub != banners[i]) {
			...
			sprintf(gamename, deh_sub, DOOM_VERSION / 100,
				DOOM_VERSION % 100);
