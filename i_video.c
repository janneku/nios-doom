...
static void LoadDiskImage(void)
{
	...
	saved_background =
	    Z_Malloc(disk_image_w * disk_image_h, PU_STATIC, NULL);

	for (y = 0; y < disk_image_h; ++y) {
		memcpy(disk_image + disk_image_w * y,
		       screens[0] + SCREENWIDTH * y, disk_image_w);
...
void I_EndRead(void)
{
	...
	for (y = 0; y < disk_image_h; ++y) {
		...
		memcpy(screenloc, saved_background + y * disk_image_w,
		       disk_image_w);
	}
...
static void I_AutoAdjustSettings(void)
{
	if (fullscreen) {
		...
		for (i = 0; modes[i] != NULL; ++i) {
			...
			screen_mode =
			    I_FindScreenMode(modes[i]->w, modes[i]->h);
		...
		printf
		    ("I_InitGraphics: %ix%i mode not supported on this machine.\n",
		...
		if (best_mode == NULL) {
			...
			best_mode =
			    I_FindScreenMode(SCREENWIDTH, SCREENHEIGHT_4_3);
		}
...
static void SetScaleFactor(int factor)
{
	if (fullscreen) {
		...
		if (best_mode == NULL) {
			I_Error
			    ("No fullscreen graphics mode available to support "
...
void I_InitGraphics(void)
{
	...
	if (screensaver_mode) {
		...
		if (screen_mode == NULL) {
			I_Error
			    ("I_InitGraphics: Unable to find a screen mode small "
		...
		if (windowwidth != screen_mode->width
			...
			printf("I_InitGraphics: %s (%ix%i within %ix%i)\n",
			       WindowBoxType(screen_mode, windowwidth,
					     windowheight), screen_mode->width,
			       screen_mode->height, windowwidth, windowheight);
		}
	...
	native_surface = !SDL_MUSTLOCK(screen)
	    ...
	    && screen->pitch == SCREENWIDTH && aspect_ratio_correct;
	...
	if (native_surface) {
		...
		screens[0] =
		    (unsigned char *)Z_Malloc(SCREENWIDTH * SCREENHEIGHT,
					      PU_STATIC, NULL);
	}
