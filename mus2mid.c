...
static boolean read_musheader(MEMFILE * file, musheader * header)
{
	...
	result = (mem_fread(&header->id, sizeof(byte), 4, file) == 4)
	    ...
	    && (mem_fread(&header->primarychannels, sizeof(short), 1, file) ==
		1)
	    && (mem_fread(&header->secondarychannels, sizeof(short), 1, file) ==
		1)
	    && (mem_fread(&header->instrumentcount, sizeof(short), 1, file) ==
		1);
	...
	while (!hitscoreend) {
		...
		while (!hitscoreend) {
			...
			case mus_releasekey:
				...
				if (midi_writereleasekey
				    (channel, key, midioutput)) {
					return true;
				}
			...
			case mus_presskey:
				...
				if (midi_writepresskey
				    (channel, key, channelvelocities[channel],
				     midioutput)) {
					return true;
				}
			...
			case mus_pitchwheel:
				...
				if (midi_writepitchwheel
				    (channel, (short)(key * 64), midioutput)) {
					return true;
				}
			...
			case mus_systemevent:
				if (mem_fread(&controllernumber, 1, 1, musinput)
				    != 1) {
					return true;
				}
				...
				if (midi_writechangecontroller_valueless
				    (channel,
			...
			case mus_changecontroller:
				...
				if (mem_fread(&controllervalue, 1, 1, musinput)
				    != 1) {
					return true;
				}

				if (controllernumber == 0) {
					if (midi_writechangepatch
					    (channel, controllervalue,
					     midioutput)) {
						return true;
					}
				} else {
					...
					if (midi_writechangecontroller_valued
					    (channel,
					     mus2midi_translation
					     [controllernumber],
